...
//Getting the response from the targeted url
    final response = await http.Client()
        .get(Uri.parse('https://verify.tra.go.tz/Home/Index'));
    ...
    if (response.statusCode == 200) {
      ...
      try {
        ...
        var responseString1 =
            document.getElementsByClassName('main_container')[0].children[0];

        print(responseString1.text.trim());

        //Scraping the second article title
        var responseString2 =
            document.getElementsByClassName('x_title')[0].children[0];

        print(responseString2.text.trim());

        //Scraping the third article title
        var responseString3 =
            document.getElementsByClassName('form-group')[0].children[0];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('TRA Receipt Scraping')),
      body: Padding(
